...
class VRM_API:
    ...
    def __init__(self, username=None, password=None, demo=False):
        ...
        if demo:  # Login as demo else with credentials
            ...
            if username and password:
                ...
                self.username = "aureqc@iracesa.com"
                self.password = "070488Aure"
    ...
    # custom stats
    def get_custom_stats(self, inst_id, start=None, end=None):
        """
        Returns the kwhs statistics for a given site
        @params - inst_id (installation id)
        @params - start ( A python datetime to start from)
        ...
        if start and end:
            data_dict = {
                ...
                'start': datetime_to_epoch(start),
                'end': datetime_to_epoch(end),
                'attributeCodes[]': ['Gb', 'Gc', 'Pb', 'Gu', 'Pg', 'Pv', 'Pc', 'Bc', 'Bg', 'gc', 'gb'],
                'interval': '15mins',
            }
        else:
            data_dict = {
